  ...
  return (
    <Card>
      <div className="mb-4 flex max-h-96 min-h-40 flex-col gap-2 overflow-y-auto">
        {messages.length === 0 && <p className="text-sm text-slate-500">Pergunte algo sobre frações.</p>}
        {messages.map((m) => (
          <p
            key={m.id}
            className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${
              m.role === 'user' ? 'self-end bg-indigo-600 text-white' : 'self-start bg-slate-100'
            }`}
          >
            {m.content}
          </p>
