    ...
    <Card title="Nova atividade">
      <form onSubmit={onSubmit} className="space-y-3 text-sm">
        <label className="block space-y-1">
          ...
          <input id="activity-title" className="w-full rounded-lg border border-slate-300 px-3 py-2" />
        </label>
        <label className="block space-y-1">
          <span className="text-slate-600">Tipo</span>
          <select id="activity-type" className="w-full rounded-lg border border-slate-300 px-3 py-2">
            {TYPES.map((t) => (
